...
class bank:
  def __init__(self):
    ...
    with open('ssBank.bin', 'rb') as ss:
      ...
      self.values = json.loads(arr[6].rstrip().lstrip())
  ...
  def handleLocal(self,inString):
    ...
    if stringParts[0].lower() == "deposit":
      if len(stringParts) != 3:
        ...
        return
      try:
        if (int(str(stringParts[2].strip())) and int(str(stringParts[2].strip())) > 0):
          self.balances[self.hashes[stringParts[1].lower()]] += int(stringParts[2])
      ...
      if len(stringParts) != 2:
        ...
        return
      try:
        if (self.hashes[stringParts[1].lower()] in self.balances):
            print("$" + str(self.balances[self.hashes[stringParts[1].lower()]]) + "\n")
      except KeyError:
        print("Account does not exist\n")
    else:
        print("Invalid command!\n")
    ...
    try:
      if inObject['operation'] == 'begin':
        if self.pins[inObject['user']] == inObject['auth']: # Throws keyerror when user not in dict
          ...
          message['user'] = self.values[inObject['user']]
          self.send(message)
        else:
        ...
        if (int(self.balances[self.hashes[inObject['user']]]) >= int(inObject['amount']) and int(inObject['amount']) > 0):
          message['success'] = True
          message['amount'] = inObject['amount']
          self.balances[self.hashes[inObject['user']]] -= inObject['amount']
          self.send(message)
        else:
        ...
        message['amount'] = self.balances[self.hashes[inObject['user']]]
        self.send(message)
      else:
        message['operation'] = 'responseError'
  ...
  def send(self, m):
    try:
      ...
      print("Error: Could not send From bank!")
  ...
  def recvBytes(self):
    ...
    if addr[0] == config.local_ip and addr[1] == config.port_router:
      try:
        ...
        print("Recv error: Error with receiving data from ATM")
    else:
